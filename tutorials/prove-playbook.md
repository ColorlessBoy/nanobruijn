...
| `hint` | 关卡内逐条显示分层提示（`提示 1/2`、`提示 2/2`……用一条少一档星级） |
| `solution` | 显示标准解并退出本关（本关不计通关、不记星） |
| `#quit` / `abort` | 中途退出当前证明 |
...
- 1★：通关（用了 hint 或步数超限都只有 1★）
- 2★：通关且没用 hint
- 3★：通关且没用 hint，且步数 ≤ 标准解步数（标准解越短，3★ 越难）
...
- **hints 设计**（2 条）：
  1. intro h1 h2 h3 —— 前提 h1 : And (a -> b) (a -> c)，h3 : a
